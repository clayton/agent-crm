...
const toolDefs: Array<{
  ...
  },
  {
    ...
    read: false,
    schema: { project: z.string(), name: z.string(), domain: z.string().optional(), website: z.string().optional(), industry: z.string().optional(), description: z.string().optional() },
  ...
  { name: "crm_update_company", description: "Update company profile fields.", read: false, schema: { company_id: z.string(), fields: z.record(z.string(), z.unknown()) }, handler: (db, actor, a) => service.updateCompany(db, a.company_id as string, actor, a.fields as Record<string, unknown>) },
  ...
  { name: "crm_update_contact", description: "Update contact fields.", read: false, schema: { contact_id: z.string(), fields: z.record(z.string(), z.unknown()) }, handler: (db, actor, a) => service.updateContact(db, a.contact_id as string, actor, a.fields as Record<string, unknown>) },
  ...
  { name: "crm_inbox", description: "Return overdue, due-soon, and stale work.", read: true, schema: { project: z.string().optional(), due_within_days: z.number().optional(), stale_days: z.number().optional() }, handler: (db, actor, a) => service.inbox(db, a.project as string | undefined, actor, (a.due_within_days as number) ?? 7, (a.stale_days as number) ?? 30) },
  { name: "crm_next_actions", description: "Return ranked actions.", read: true, schema: { project: z.string(), limit: z.number().optional(), stale_days: z.number().optional(), mode: z.string().optional(), time_budget: z.number().optional() }, handler: (db, actor, a) => service.nextActions(db, a.project as string, actor, (a.limit as number) ?? 5, (a.stale_days as number) ?? 30, (a.mode as string) ?? "balanced", a.time_budget as number | undefined) },
